...
use sroze\voronoi\Nurbs\Exceptions\VoronoiException;
//
...
class Voronoi
{
    ...
    public function addBeachsection ($site) 
    {
        ...
        while ($node) {
            ...
            if ($dxl > self::EPSILON) {
                if (!$node->left) {
                    throw new VoronoiException("should not happen");
                } else {
                    $node = $node->left;
                }
            }
    ...
    public function closeCells ($bbox) 
    {
        ...
        while ($iCell--) {
            ...
            while ($iLeft < $nHalfedges) {
                $va = $cell->_halfedges[$iLeft]->getEndpoint();
                $vz = $cell->_halfedges[($iLeft+1) % $nHalfedges]->getStartpoint();
                // if end point is not equal to start point, we need to add the missing
                // halfedge(s) up to vz
                if (abs($va->x-$vz->x)>=self::EPSILON || abs($va->y-$vz->y)>=self::EPSILON) {
                    ...
                    switch (true) {
                        ...
                        case $this->equalWithEpsilon($va->x,$xl) && $this->lessThanWithEpsilon($va->y,$yb):
                            ...
                            $iLeft++;
                            array_splice($cell->_halfedges,$iLeft, 0, array(new Halfedge($edge, $cell->_site, null)));
                            $nHalfedges++;
                        ...
                        default:
                            throw new VoronoiException("Voronoi.closeCells() > this makes no sense!");
